...
__modified__ = datetime.datetime(2015, 10, 12)
__version__  = "1.0"
__status__   = "Production"

def docs():
	"""
	This program extracts data from specific locations in the grid file defined in the xpos, ypos arrays. These positions are
	converted to longitude-latitude and the equvivalent points are extracted from the global GLORYS2V3 dataset. The data from 
	both the ROMS and the GLORYS2V3 models are plotted together to create a comparison of accuracy. 

	This script is run on Hexagon.
	"""

def setupSystem():
	print "Initializing (func:setupSystem)"
	
	myremotedir="/work/users/trondk/KINO/FORWARD/Run/RESULTS/"
	mypattern = "kino_1600m*" #"kino_1600m_*[1234]*.nc"
	mystartYear=2010
	mylocaldir="/Users/trondkr/Projects/KINO/STATUS/"
	mygridfile="/work/users/trondk/KINO/GRID/kino_1600m_02122015_vf20.nc"
...
def getLocations(mygridfile):
	...
	return xpos,ypos,xynames,lonpos,latpos, mask
...
def getValuesFromKINO(xpos,ypos,xynames,datapath,mypattern,debug):
	
	print "Extracting values from KINO (func:getValuesFromKINO)"
	...
	print "Sorting %s files found in KINO datadirectory"%(len(allFiles))

	refdate=datetime.datetime(1948,1,1)
		
	# Get the total number of timesteps in all files. Timesteps may vay between files.
	allTimes=[]
	for afile in allFiles:
		
		cdf = Dataset(afile)
		times = cdf.variables["ocean_time"][:]
		allTimes.append(len(times))
		cdf.close()
	totalNumberOfTimeSteps=np.sum(allTimes)

 	# Get the data for each file and timestep
	for afile in allFiles:
		
		cdf = Dataset(afile)
		times = dataSST = cdf.variables["ocean_time"][:]
		if first:
			allValues=np.zeros(shape=(len(xpos),totalNumberOfTimeSteps,len(variables)))
			allDates=[]
			first = False
...
def createTimeseriesPlot(stations,dataKINO,datesKINO,dataGLORYS2V3,datesGLORYS2V3,startDate,endDate,plotfileName):
	
	print "Creating timeseries plot (func:createTimeseriesPlot)"
	dates1 = matplotlib.dates.date2num(datesKINO)
	...
	plots=[]
  	for i,station in enumerate(stations):
  		# SST
		myplt,=plt.plot_date(dates1, dataKINO[i,:,0],fmt=fmtNS[i],markersize=3,xdate=True,ydate=False)
		plots.append(myplt)
	...
	ax2 = setupSubPlot(subplotIndex=2)
  	for i,station in enumerate(stations):
  		# SSS
		plt.plot_date(dates1, dataKINO[i,:,1],fmt=fmtNS[i],markersize=3,xdate=True,ydate=False)
...
def main():
	...
	xpos,ypos,xynames,lonpos,latpos,mask = getLocations(mygridfile)
	...
	allGLORYS2V3Values, allGLORYS2V3Dates = getValuesFromGLORYS(glorysIndicies,myglorys2v3pathT,myglorys2v3pathS,xynames,mystartYear,debug)
 	allKINOValues, allKINODates = getValuesFromKINO(xpos,ypos,xynames,myremotedir,mypattern,debug)

 	# Create plot for date range of GLORYS2V3 defined dates
 	createTimeseriesPlot(xynames,allKINOValues,allKINODates,allGLORYS2V3Values,allGLORYS2V3Dates,allGLORYS2V3Dates[0],allGLORYS2V3Dates[-1],"timeseries_KINO1600M_vs_GLORYS2V3.png")

 	# Create zoomed plot for date range of KINO defined dates
 	createTimeseriesPlot(xynames,allKINOValues,allKINODates,allGLORYS2V3Values,allGLORYS2V3Dates,allKINODates[0],allKINODates[-1],"timeseries_KINO1600M_vs_GLORYS2V3_zoomed.png")
