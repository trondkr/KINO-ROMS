Module bioenergetics
  ...
  ! ---------------------------------------------------------------
  !
  ! This a simplified version of the model found here: github.com/trondkr/KINO-ROMS/OPENDRIFT/kino/bioenergetics.f90
  ! For questions on this version: kristokv@gmail.com
  ! ---------------------------------------------------------------------------------------------------------------------------------------------------
...
!f2py intent(in) haddock,activemetabOn,constantIngestion,swimdistance,Eb,deltaH,dt,temp
    ...
    real(kind=8),parameter:: sec2hours = 1.0/3600
    real(kind=8),parameter:: gut_size=0.05
    real(kind=8),parameter:: costRateOfMetabolism=1/100000  !(NOTE: NOT USED IN NEW GROWTH MODEL, KK)
    ...
    if (haddock==1) then
        ...
        meta=(dt*sec2hours)*(0.00114*((larvawgt*mg2ug)**(1.029-0.00774&
    ...
    !Calculate assimilation efficiency (NOTE: NOT USED IN NEW GROWTH MODEL, KK)
    ...
    maxg =  max(0.0,(dlog(maxgrowthrateInPercent/100.+1)))
    
    maxgrowthrateInMG=(larvawgt*(Exp(maxg)-1.))*sec2day*dt
    ...
    ! Calculate how far the larva can swim in one timestep (NOTE: NOT USED IN NEW GROWTH MODEL, KK)
    ...
    !activityCost= meta
    larvawgt=larvawgt + maxgrowthrateInMG
